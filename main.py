...
def main(max_size, colors=True):
    ...
    definitions = svg.Defs(elements=[])
    for rotation in range(0, 360, 90):
        definitions.elements.extend([
            get_straight(f'straight_out_{rotation:03}', colors, [svg.Rotate(rotation)]),
            get_straight(f'straight_in_{rotation:03}', colors[::-1], [svg.Rotate(rotation)]),
            get_corner(f'corner_out_{rotation:03}', colors, [svg.Rotate(rotation)]),
            get_corner(f'corner_in_{rotation:03}', colors[::-1], [svg.Rotate(rotation)]),
        ])
